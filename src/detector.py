...
class YOLOv5(nn.Module):
    def __init__(self, device):
        ...
        from detlib.yolov5master.models.experimental import attempt_load
        sys.path.remove(root)

        super().__init__()
        self.model = attempt_load(f"detlib/weights/yolov5m.pt", device=device, fuse=True)
...
class YOLOv3(nn.Module):
    def __init__(self, device):
        root = os.getcwd()
        sys.path.insert(0, root)
        from detlib.yolov3master.models.experimental import attempt_load
        sys.path.remove(root)

        super().__init__()
        self.model = attempt_load(f"detlib/weights/yolov3.pt", device=device, fuse=True)
        for k in self.model.model.children():
            if "Detect" in str(type(k)):
                k.inplace = False
        self.conf_thres = 0.25
...
class FasterRCNN(nn.Module):
    def __init__(self, device):
        ...
        self.model.load_state_dict(torch.load("detlib/weights/fasterrcnn.pth"))
    ...
    def forward(self, x, y=None, hiding=False):
        if y is None:
            ...
            proposals, scores = self.model.rpn.filter_proposals(proposals, objectness, x.image_sizes,
